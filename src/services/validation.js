// Validation services and utilities (JS version)
// TODO: Replace mock implementations with real API calls

// Types (for reference)
// type AM = { nik_am: string; id_sales?: string; nama_am?: string; region?: string; witel?: string; updated_at?: string }
// type TempRow = AM & { sumber: 'CA' | 'ATM'; status: 'valid' | 'tidak valid'; ts: string }
// type LogItem = { id: string; actor: string; action: 'VALIDATE_AM'|'VALIDATE_KARYAWAN'|'GENERATE'|'SYNC'|'CANCEL'; count?: number; duration_ms?: number; ts: string }

export async function fetchATM() {
...
export function diffKaryawanToATM(karyawan, atm) {
  return diffCAtoATM(karyawan, atm)
}

export async function runValidateAM(actor) {
...
export async function runValidateKaryawan(actor) {
  const t0 = performance.now()
...
export async function generateCommit(actor) {
  return {
    id: crypto.randomUUID(),
    actor,
    action: 'GENERATE',
...
export async function syncMasters(actor) {
  return {
    id: crypto.randomUUID(),
    actor,
    action: 'SYNC',
...
export async function cancelValidation(actor) {
  return {
    id: crypto.randomUUID(),
    actor,
    action: 'CANCEL',
